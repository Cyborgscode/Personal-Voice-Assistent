...
//					log("key="+ key +"\nvalue="+ value );
	...
	static public String filterAIThinking(String answere) {
		...
		answere = answere.replaceAll("\\*\\*","").replaceAll("\\*","");
		
		return answere
			.replace("\\", "\\\\")    // Maskiert Backslashes
		        .replace("\"", "\\\"")    // Maskiert doppelte Anführungszeichen
		        .replace("\b", "\\b")
		        .replace("\f", "\\f")
		        .replace("\n", "\\n")    // Verhindert echte Zeilenumbrüche im JSON-String
		        .replace("\r", "\\r")
		        .replace("\t", "\\t");
	}
