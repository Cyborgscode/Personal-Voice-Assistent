...
//					log("no internal reaction yet, lets test plugins to handle it.");	
				...
				if ( !dos.fileExists( getHome() + "/.local/tmp/donotbark") ) {
...
//      	  	if ( debug > 4 ) log("["+ tk.get("proc","counter") +"] Analyse mp3 ... "+ filename );
	...
	static public String getText(String key) {
		try {
			...
			// we get here, because "pls" is null, because pluginloading is not finished and a plugin wanted a localized text
		}	
